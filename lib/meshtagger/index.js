...
angular.module(moduleName, [ angularMaterial ])
  ...
  .controller(controllerName, ['$scope', '$window', '$mdDialog', '$timeout', function($scope, $window, $mdDialog, $timeout) {
    ...
    function setBusy() {
        angular.element('.busy-spinner-indicator').css('display', 'block');
    }

    function unsetBusy() {
        angular.element('.busy-spinner-indicator').css('display', 'none');
    }
    ...
    $scope.connect = function(url, appKey) {
      ...
      vtkWeb.smartConnect(configObject,
         function(connection) {
            ...
            if ($scope.state) {
                var myState = $scope.state.load();
                if (myState.hasOwnProperty('faces')) {
                    $scope.faces = myState.faces;
                    stateLoaded = true;
                }
            ...
            function setupColoring() {
                var totalNumberOfElements = $scope.faces.length + $scope.blocks.length, currentlyRecoloredCount = 0;
    ...
    $scope.toggleControlTab = function() {
        ...
        angular.element('.control-tab').toggleClass('open closed');
        if ($scope.showingControlTab) {
            angular.element('.hydra-mesh-viewer > .control-panel').css('border-radius', '5px 5px 0 0');
        } else {
            angular.element('.hydra-mesh-viewer > .control-panel').css('border-radius', '5px');
        }
    };
    ...
    $scope.toggleBackgroundColor = function() {
        ...
        session.call('toggle.bg.color', []).then(function(newBgColor){
            ...
            if (newBgColor[0] < 0.5) {
                angular.element('.busy-spinner-indicator').css('color', 'white');
                angular.element('.hydra-mesh-viewer > .control-panel').css('background-color', '#aaaaaa').css('color', 'black');
                angular.element('.hydra-mesh-viewer > .control-tab.open').css('border', 'none');
            } else {
                angular.element('.busy-spinner-indicator').css('color', 'black');
                angular.element('.hydra-mesh-viewer > .control-panel').css('background-color', '#444444').css('color', 'white');
                if (Math.abs(newBgColor[0] - 1.0) <= 0.000001) {
                    angular.element('.hydra-mesh-viewer > .control-tab.open').css('border', '1px solid gray');
                } else {
                    angular.element('.hydra-mesh-viewer > .control-tab.open').css('border', 'none');
                }
            }
